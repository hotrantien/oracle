...
import logging
import oci
availabilityDomains = ["KBzp:AP-SINGAPORE-1-AD-1"]
displayName = '424'
compartmentId = 'ocid1.tenancy.oc1..aaaaaaaa6y4eoshlvky4rljwwchmksfne6fwjw7aeymyy5emdmmtlwczts6a'
subnetId = 'ocid1.subnet.oc1.ap-singapore-1.aaaaaaaaq2naayvtjrhrk24ocmrwcw5nogrmvi4ze4ob4opd6b2mlrmzafda'
...
imageId = "ocid1.image.oc1.ap-singapore-1.aaaaaaaahujcmwrw6x7uprliju64sma3xodmfmqgzxn5lhftgkqiqjjvvtyq"
boot_volume_size_in_gbs = 100
boot_volume_id = "xxxx"

bot_token = "6668962371:AAGRMybrGe9EjzVFjyEX90b88dMSlzJW45s"
uid = "5244987499"
...
if imageId != "xxxx":
    try:
        list_volumes = volume_client.list_volumes(
            compartment_id=compartmentId).data
    except Exception as e:
        logging.info(f"{e.status} - {e.code} - {e.message}")
        logging.info(
            "Error detected. Stopping script. Check values of bot.py file, config file and oci_private_key.pem file. If you can't fix error, contact me.")
        sys.exit()
    if list_volumes != []:
    ...
    for availabilityDomain in availabilityDomains:
        list_boot_volumes = volume_client.list_boot_volumes(
            availability_domain=availabilityDomain, compartment_id=compartmentId).data
    ...
    if boot_volume_size_in_gbs == "xxxx" and free_storage < 47:
        logging.critical(
            f"There is {free_storage} GB out of 200 GB left in your oracle cloud account. 47 GB storage needs to create this vps. So, you need to delete unnecessary boot and block volumes in your oracle cloud account. **SCRIPT STOPPED**")
        sys.exit()
...
if imageId != "xxxx":
    if boot_volume_size_in_gbs == "xxxx":
        ...
        op = oci.core.models.InstanceSourceViaImageDetails(
            source_type="image", image_id=imageId, boot_volume_size_in_gbs=boot_volume_size_in_gbs)
